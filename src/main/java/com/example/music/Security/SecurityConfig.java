...
public class SecurityConfig {
    ...
    private final String issuerUri = "https://dev-org1gkrj5i6kdrdr.us.auth0.com/";
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/auth/**").permitAll()
                        .requestMatchers("/api/merchandise/getAll", "/api/track/getAll", "/api/events/getAll").permitAll()
                        .requestMatchers(HttpMethod.POST, "/api/events/create").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/merchandise/upload").hasRole("ADMIN")
                        .anyRequest().authenticated()
                )
    ...
    public JwtAuthenticationConverter jwtAuthenticationConverter() {
        ...
        converter.setJwtGrantedAuthoritiesConverter(jwt -> {
            ...
            List<String> roles = jwt.getClaimAsStringList("https://music-app/roles");

            if (roles == null) return scopes;

            List<SimpleGrantedAuthority> roleAuthorities = roles.stream()
                    .map(role -> new SimpleGrantedAuthority("ROLE_" + role))
                    .toList();

            return Stream.concat(scopes.stream(), roleAuthorities.stream()).toList();
        });
